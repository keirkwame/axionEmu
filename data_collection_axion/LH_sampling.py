...
H_0 =         np.linspace(39.99, 100.01, n_samples) #64., 68., n_samples) #55.0,    82.0,    n_samples) 
ns =        np.linspace(0.8, 1.2, n_samples) #0.95, 0.98, n_samples) #0.9, 1.1 #0.86, 1.07,    n_samples)
As =      np.linspace(1.2182493960703474e-09, 3.311545195869231e-09, n_samples) #2.06e-9, 2.2e-9, n_samples) #1.3e-9, 2.8e-9 #5e-10,    2.6e-9,    n_samples)
tau_reio = np.linspace(0.02, 0.12, n_samples) #0.043, 0.069, n_samples) #0.01, 0.1 #0.02, 0.12,    n_samples)
...
sum_omega = np.linspace(0.08, 0.2, n_samples) #0.12, 0.13, n_samples) #0.09, 0.15, n_samples) # sum of omaxh2 and omlambda in dark-energy region #sum of omaxh2 and omch2 in dark-matter region
#0.124, 0.565
