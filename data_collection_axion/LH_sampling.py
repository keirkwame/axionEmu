...
# parameter ranges
obh2 =      np.linspace(0.022383, 0.022383, n_samples) #0.021, 0.024, n_samples) #0.01933, 0.02533, n_samples) #0.0222, 0.0229, n_samples) #0.017, 0.027, n_samples)
H_0 =         np.linspace(67.32, 67.32, n_samples) #62., 75., n_samples) #39.99, 100.01, n_samples) #64., 68., n_samples) #55.0,    82.0,    n_samples) 
ns =        np.linspace(0.96605, 0.96605, n_samples) #0.92, 1.01, n_samples) #0.8, 1.2, n_samples) #0.95, 0.98, n_samples) #0.86, 1.07,    n_samples)
As =      np.linspace(2.1005e-9, 2.1005e-9, n_samples) #1.5e-9, 2.5e-9, n_samples) #1.2182493960703474e-09, 3.311545195869231e-09, n_samples) #2.06e-9, 2.2e-9, n_samples) #1.3e-9, 2.8e-9 #5e-10,    2.6e-9,    n_samples)
tau_reio = np.linspace(0.0543, 0.0543, n_samples) #0.02, 0.09, n_samples) #0.02, 0.12, n_samples) #0.043, 0.069, n_samples) #0.01, 0.1 #0.02, 0.12,    n_samples)
...
sum_omega = np.linspace(0.12011, 0.12011, n_samples) #0.1, 0.14, n_samples) #0.08, 0.2, n_samples) #0.12, 0.13, n_samples) #0.09, 0.15, n_samples) # sum of omaxh2 and omlambda in dark-energy region #sum of omaxh2 and omch2 in dark-matter region
#0.124, 0.565
f_ax = np.linspace(1.e-8, 1.e-8, n_samples) #1
...
data_pkl = '/home/keir/keir/LH_ACT_DR6_TTTEEEPP_derived_120_Planck_gfortran_orig_fast.pkl' #the .pkl that stores all input parameters
f = open(data_pkl, 'wb')
...
for i in range(num_subfile):
    ...
    data_pkl = '/home/keir/keir/LH_ACT_DR6_TTTEEEPP_derived_Planck_LCDM_gfortran_orig_fast_' +str(i) +'.pkl'
    f = open(data_pkl, 'wb')
