# -*- coding: utf-8 -*-
"""Copy of Copy of 1e5_emulator_TT.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1yeG7V6XP6ExwrHxZJp5EiFSL7b9UR5DC
"""
...
spectra_= np.array(C_phi_list)/(7.4311*10**(12))
print(spectra_.shape)
spectra_ = spectra_[:,:5999]/(ell_range**4)
...
with tf.device('/device:GPU:0'): # ensures we are running on a GPU
    ...
    cp_nn.train(training_parameters=training_parameters,
                ...
                filename_saved_model='Phi_cp_NN_5e5_t2',
                # cooling schedule
...
cp_nn = cosmopower_NN(restore=True, 
                      restore_filename='Phi_cp_NN_5e5_t2',
                      )
...
for i in range(3):
    pred = predicted_testing_spectra[i]*(ell_range**4)
    true = testing_spectra[i]*(ell_range**4)
    ...
    ax[i].set_ylabel('$\ell^{4} C_\ell$', fontsize='x-large')
    ax[i].legend(fontsize=15)
plt.savefig('examples_reconstruction_phi_5e5_2.pdf')

# load noise models from the SO noise repo
noise_levels_load = np.loadtxt('./so_noise_models/LAT_lensing_noise/lensing_v3_1_1/nlkk_v3_1_0_deproj0_SENS2_fsky0p4_it_lT30-3000_lP30-5000.dat')
conv_factor = 1

ells = noise_levels_load[:, 0]
SO_TT_noise = noise_levels_load[:, 7][:4999] / conv_factor
new_ells = ells[:4999]
SO_TT_noise = (4*SO_TT_noise)/((new_ells*(new_ells+1))**2)

f_sky = 0.4
prefac = np.sqrt(2/(f_sky*(2*new_ells+1)))
denominator = prefac*(testing_spectra[:,:4999]+SO_TT_noise)  # use all of them
diff = np.abs((predicted_testing_spectra[:,:4999] - testing_spectra[:,0:])/(denominator))
...
plt.ylabel(r'$\frac{| C_{\ell, \rm{emulated}}^{\rm{\phi\phi}} - C_{\ell, \rm{true}}^{\rm{\phi\phi}}|} {\sigma_{\ell, \rm{CMB}}^{\rm{\phi\phi}}}$', fontsize=50)
...
plt.tight_layout()
plt.savefig('./accuracy_emu_phi_wide_5e5_t2_no_conv_factor.pdf')
...
plt.savefig('bad_params_5e5_2_phi.pdf')
noise_levels_load = np.loadtxt('./so_noise_models/LAT_lensing_noise/lensing_v3_1_1/nlkk_v3_1_0_deproj0_SENS2_fsky0p4_it_lT30-3000_lP30-5000.dat')
conv_factor = 7.4311e12
...
plt.setp(ax.get_yticklabels(), fontsize=25)
plt.tight_layout()
plt.savefig('./accuracy_emu_phi_wide_5e5_t2_7.4311e12.pdf')
