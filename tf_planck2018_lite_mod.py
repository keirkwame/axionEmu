...
# ===== INIT ==========
    def __init__(self, 
        ...
        self.fix_parameters = fix_parameters #MOD
        # Attributes
        ...
        self.fix_parameter_keys = list(self.fix_parameters.keys()) #MOD
...
# ===== INVERSE COVARIANCE ======
    ...
    def from_parameters_tensor_to_table(self,
        ...
        if self.fix_parameter_keys != []: #MOD
            for k in self.fix_parameter_keys:
                fix_parameters_values=list(self.fix_parameters[k]*np.ones([parameters_tensor.shape[0]]))
                parameters_values = tf.concat([parameters_values, tf.constant([fix_parameters_values], dtype=tf.float32)],axis=0) 
            parameters_table.insert(self.parameter_keys+self.fix_parameter_keys, parameters_values) 
        else:
            parameters_table.insert(self.parameter_keys, parameters_values)
        if 'omega_cdm' not in self.parameter_keys:
            h_values = parameters_values[2]
            omega_b_values = parameters_values[0]
            omega_nu_values = list(0.0006*np.ones([parameters_tensor.shape[0]]))
            omega_nu_values = tf.constant(omega_nu_values, dtype=tf.float32)
            omega_lamda_values = parameters_values[1]
            omega_ax_values = parameters_values[6]
            omega_sum = tf.add(omega_ax_values, omega_lamda_values)
            omega_sum = tf.add(omega_sum, omega_b_values)
            omega_sum = tf.add(omega_sum, omega_nu_values)
            omega_cdm_values = tf.subtract(tf.multiply(h_values,h_values), omega_sum)
            parameters_table.insert(['omega_cdm'], [omega_cdm_values])
        #print("new parameters_values shape", parameters_values.shape)
    ...
    def get_loglkl(self, parameters):
        ...
        Cltt= self.tt_emu_model.ten_to_predictions_tf(cosmo_params)[:,:2507] #CHANGE!!
        Clte= self.te_emu_model.predictions_tf(cosmo_params)[:,:2507]
        Clee= self.ee_emu_model.ten_to_predictions_tf(cosmo_params)[:,:2507]
        # Cltt= self.tt_emu_model.ten_to_predictions_tf(cosmo_params) #CHANGE!!
