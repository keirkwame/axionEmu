...
import numpy as np
from test_log_likelihood import log_likelihood, log_likelihood_ACT_DR4
...
def log_posterior(x, parameters_and_priors):
    ...
    if log_prior_value == -np.inf:
        ...
        log_likelihood_value = log_likelihood_ACT_DR4(x)
...
FIDUCIAL = np.reshape(np.array([ 0.0215,  0.05,  0.673,  0.065,  1.0,  3.0753, 0.05, 1.0]).astype('float32'), (1, 8))
...
with Pool(processes=60) as pool:
    ...
    diff = end - start
    data_pkl = 'ACT_DR4_axionCAMB_8500'+'.pkl'
    ...
    plt.savefig('convergence_test_ACT_DR4.png')
